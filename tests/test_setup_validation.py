...
def test_report_direwolf_log_summary_missing(
    ...
    monkeypatch.setattr(
        "neo_aprs.commands.setup.config_module.get_data_dir", lambda: tmp_path
    )
...
def test_can_launch_direwolf_true(monkeypatch) -> None:
    monkeypatch.setattr(
        "neo_aprs.commands.setup.shutil.which", lambda cmd: f"/usr/bin/{cmd}"
    )
...
def test_can_launch_direwolf_false(monkeypatch) -> None:
    monkeypatch.setattr(
        "neo_aprs.commands.setup.shutil.which",
...
def test_launch_direwolf_probe_missing_config(
    ...
    monkeypatch.setattr(
        "neo_aprs.commands.setup.config_module.get_config_dir", lambda: tmp_path
    )
...
def test_launch_direwolf_probe_success(tmp_path: Path, monkeypatch, caplog) -> None:
    ...
    monkeypatch.setattr(
        "neo_aprs.commands.setup.config_module.get_data_dir", lambda: data_dir
    )
    monkeypatch.setattr(
        "neo_aprs.commands.setup.config_module.get_config_dir", lambda: config_dir
    )
    ...
    monkeypatch.setattr("neo_aprs.commands.setup.subprocess.Popen", fake_popen)
