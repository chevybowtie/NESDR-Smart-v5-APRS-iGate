...
def test_report_direwolf_log_summary_missing(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    assert "Direwolf log not found" in captured.out


def test_report_direwolf_log_summary_includes_recent_lines(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    assert "[OK     ] Direwolf log found" in captured.out
    # Only the last six lines should be printed
    for index in range(2, 8):
        assert f"line {index}" in captured.out
...
def test_launch_direwolf_probe_missing_config(tmp_path: Path, monkeypatch, capsys) -> None:
    monkeypatch.setattr("nesdr_igate.commands.setup.config_module.get_data_dir", lambda: tmp_path)
...
def test_launch_direwolf_probe_success(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    assert "Frame decoded" in captured.out
