...
def test_run_setup_reset_removes_config(tmp_path: Path, monkeypatch, capsys) -> None:
    config_path = tmp_path / "config.toml"

    monkeypatch.setattr(config_module, "_store_passcode_in_keyring", lambda *args, **kwargs: None)
    monkeypatch.setattr(config_module, "_retrieve_passcode_from_keyring", lambda *_: "12345")
    delete_calls: list[str] = []
    monkeypatch.setattr(config_module, "delete_passcode_from_keyring", lambda callsign: delete_calls.append(callsign))

    cfg = StationConfig(callsign="N0CALL-10", passcode="12345", passcode_in_keyring=True)
    save_config(cfg, path=config_path)

    args = Namespace(config=str(config_path), reset=True, non_interactive=True, dry_run=False)
    exit_code = setup.run_setup(args)
    captured = capsys.readouterr()

    assert exit_code == 1
    assert "Removed existing configuration" in captured.out
...
def test_interactive_prompt_keyring_store(monkeypatch) -> None:
    ...
    monkeypatch.setattr(setup.config_module, "delete_passcode_from_keyring", lambda *_: None)
...
def test_interactive_prompt_keyring_missing_backend(monkeypatch, capsys) -> None:
    ...
    assert "keyring backend is unavailable" in captured.out
    assert config.passcode_in_keyring is False
    assert config.passcode == "secret"
    assert config.callsign == "N0DEV-2"


def test_prompt_yes_no_with_invalid_response(monkeypatch, capsys) -> None:
...
def test_maybe_render_direwolf_config_template_missing(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    assert "Direwolf template unavailable" in captured.out
    assert not (tmp_path / "direwolf.conf").exists()


def test_maybe_render_direwolf_config_existing_decline(tmp_path: Path, monkeypatch, capsys) -> None:
    monkeypatch.setattr(setup, "_load_direwolf_template", lambda: "CALLSIGN {{{CALLSIGN}}}")
    cfg = StationConfig(callsign="N0CALL-1", passcode="12345", aprs_server="example", aprs_port=12345)
    dest = tmp_path / "direwolf.conf"
    dest.write_text("existing", encoding="utf-8")
    monkeypatch.setattr(setup.config_module, "get_data_dir", lambda: tmp_path / "data")

    prompts: list[str] = []
    monkeypatch.setattr(setup, "_prompt_yes_no", lambda message, default: prompts.append(message) or False)

    setup._maybe_render_direwolf_config(cfg, tmp_path)
    captured = capsys.readouterr()

    assert "Keeping existing" in captured.out
...
def test_run_hardware_validation_reports(monkeypatch, capsys) -> None:
    ...
    def fake_probe(host: str, port: int, timeout: float):  # type: ignore[no-untyped-def]
        if host == "127.0.0.1":
            return type("Result", (), {"success": True, "latency_ms": 12.3, "error": None})()
        return type("Result", (), {"success": False, "latency_ms": None, "error": "timeout"})()
...
def test_run_hardware_validation_launches_probe(monkeypatch, capsys) -> None:
    ...
    monkeypatch.setattr(
        setup,
        "probe_tcp_endpoint",
        lambda *_, **__: type("Result", (), {"success": True, "latency_ms": 1.0, "error": None})(),
    )
    monkeypatch.setattr(setup, "_report_direwolf_log_summary", lambda: None)
