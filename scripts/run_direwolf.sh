...
# logs are captured under ~/.local/share/nesdr-igate/logs by default.

set -euo pipefail

CONFIG_PATH=${CONFIG_PATH:-$HOME/.config/nesdr-igate/direwolf.conf}
AUDIO_SAMPLE_RATE=${AUDIO_SAMPLE_RATE:-22050}
GAIN=${GAIN:-35}
FREQUENCY_HZ=${FREQUENCY_HZ:-144390000}
RTL_DEVICE_INDEX=${RTL_DEVICE_INDEX:-0}
RTL_PPM=${RTL_PPM:-0}
LOG_DIR=${LOG_DIR:-$HOME/.local/share/nesdr-igate/logs}
...
Environment overrides:
  CONFIG_PATH        Path to Direwolf config (default: ~/.config/nesdr-igate/direwolf.conf)
  ...
  LOG_DIR            Directory for Direwolf logs (default: ~/.local/share/nesdr-igate/logs)
...
if [[ ! -f "$CONFIG_PATH" ]]; then
  ...
  echo "Render the template in docs/templates/direwolf.conf or rerun 'nesdr-igate setup'." >&2
  exit 1
fi
