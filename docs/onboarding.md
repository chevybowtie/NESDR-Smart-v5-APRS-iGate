...
- Persist configuration to `~/.config/nesdr-igate/config.toml` while protecting sensitive data.
...
5. **Configuration Persistence**
   - Store collected data in `~/.config/nesdr-igate/config.toml` with mode `0o600`.
   - Passcode handling:
   ...
   - Include derived settings: detected tuner gain, sample rate, KISS host/port, timestamp of onboarding.
      - Render Direwolf config from `docs/templates/direwolf.conf` into `~/.config/nesdr-igate/direwolf.conf` when user opts in to managed setup.
...
6. **Telemetry & Logging Setup**
   ...
   - Ensure `~/.local/share/nesdr-igate/logs` exists with rotation policy notes.
...
8. **Completion Summary**
   ...
   - Provide next command suggestions (`nesdr-igate listen`, `nesdr-igate diagnostics`).

## CLI Options
- `nesdr-igate setup`: interactive wizard (default).
- `nesdr-igate setup --reset`: delete existing config (confirm first) and rerun wizard.
- `nesdr-igate setup --non-interactive --config /path/to/file`: accept pre-filled TOML, only validate hardware/software.
- `nesdr-igate setup --dry-run`: perform validation without writing config changes.
...
## Artifacts
- `~/.config/nesdr-igate/config.toml`: primary config file.
- `~/.config/nesdr-igate/direwolf.conf`: rendered Direwolf configuration (optional, managed flow).
- Keyring entry (`nesdr-igate/callsign`): secure passcode storage when available.
- `~/.local/share/nesdr-igate/logs/setup.log`: onboarding transcript for diagnostics.
